"""Unit tests for everos_cloud.lib._upload — batch_sign and s3_post_upload."""
...
from everos_cloud.lib._files import ResolvedFile
from everos_cloud.lib._detect import UploadTask
from everos_cloud.lib._errors import UploadError
from everos_cloud.lib._upload import (
    _RETRY_BACKOFF_S,
    _MAX_UPLOAD_RETRIES,
    SignedFile,
    batch_sign,
...
def test_s3_post_upload_success_204(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.post", return_value=_mock_httpx_response(204)) as mock_post:
...
def test_s3_post_upload_success_200(tmp_path: Path) -> None:
    signed = _make_signed_file(tmp_path)
    with patch("everos_cloud.lib._upload.httpx.post", return_value=_mock_httpx_response(200)):
        result = s3_post_upload(signed)
    assert result.object_key == signed.object_key
...
def test_s3_post_upload_4xx_no_retry(tmp_path: Path) -> None:
    signed = _make_signed_file(tmp_path)
    with patch("everos_cloud.lib._upload.httpx.post", return_value=_mock_httpx_response(403)) as mock_post:
...
def test_s3_post_upload_5xx_retries_and_fails(tmp_path: Path) -> None:
    signed = _make_signed_file(tmp_path)
    with patch("everos_cloud.lib._upload.httpx.post", return_value=_mock_httpx_response(503)) as mock_post:
        with patch("everos_cloud.lib._upload.time.sleep"):
...
def test_s3_post_upload_5xx_then_success(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.post", side_effect=responses) as mock_post:
        with patch("everos_cloud.lib._upload.time.sleep"):
            result = s3_post_upload(signed)
...
def test_s3_post_upload_timeout_retries(tmp_path: Path) -> None:
    signed = _make_signed_file(tmp_path)
    with patch(
        "everos_cloud.lib._upload.httpx.post",
...
def test_s3_post_upload_sync_backoff_called(tmp_path: Path) -> None:
    ...
    signed = _make_signed_file(tmp_path)
    with patch("everos_cloud.lib._upload.httpx.post", return_value=_mock_httpx_response(503)):
        with patch("everos_cloud.lib._upload.time.sleep") as mock_sleep:
...
async def test_async_s3_post_upload_success(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.AsyncClient", return_value=mock_http):
...
async def test_async_s3_post_upload_5xx_retries_all_fail(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.AsyncClient", return_value=mock_http):
        with patch("everos_cloud.lib._upload.asyncio.sleep", new_callable=AsyncMock):
            with pytest.raises(UploadError, match="after 3 attempts"):
                await async_s3_post_upload(signed)
...
async def test_async_s3_post_upload_backoff_called(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.AsyncClient", return_value=mock_http):
        with patch("everos_cloud.lib._upload.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            with pytest.raises(UploadError):
                await async_s3_post_upload(signed)
