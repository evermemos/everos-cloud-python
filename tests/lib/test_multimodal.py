"""Unit tests for everos_cloud.lib._multimodal — orchestration and resource override."""
...
from everos_cloud.lib._errors import UploadError, MultimodalError
from everos_cloud.lib._upload import UploadResult
from everos_cloud.lib._multimodal import (
...
def test_sync_add_local_file_upload(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.post") as mock_s3:
...
def test_sync_add_deepcopy_original_not_mutated(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._upload.httpx.post", return_value=MagicMock(status_code=204)):
...
def test_sync_add_cleanup_on_success(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._multimodal._cleanup") as mock_cleanup:
        with patch("everos_cloud.lib._upload.httpx.post", return_value=MagicMock(status_code=204)):
            with patch.object(MemoriesResourceWithMultimodal.__bases__[0], "add", return_value=_make_add_response()):
                resource.add(messages=msgs, user_id="u1")  # type: ignore[arg-type]
...
def test_sync_add_cleanup_on_upload_failure(tmp_path: Path) -> None:
    ...
    with patch("everos_cloud.lib._multimodal._cleanup") as mock_cleanup:
        with patch("everos_cloud.lib._upload.httpx.post", return_value=MagicMock(status_code=500)):
