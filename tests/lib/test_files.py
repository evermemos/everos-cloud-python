"""Unit tests for everos_cloud.lib._files — FileInput, ResolvedFile, resolve_file."""
...
from everos_cloud.lib._files import (
    FileInput,
    ResolvedFile,
    resolve_file,
    async_resolve_file,
)
from everos_cloud.lib._errors import FileResolveError
...
def test_resolve_file_url_downloads_to_temp() -> None:
    ...
    with patch("everos_cloud.lib._files.httpx.Client", return_value=mock_client):
        rf = resolve_file(fi)
...
def test_resolve_file_url_cleans_up_temp_on_error() -> None:
    ...
    with patch("everos_cloud.lib._files.httpx.Client", return_value=mock_client):
        with patch("everos_cloud.lib._files.tempfile.mkstemp", side_effect=spy_mkstemp):
...
async def test_async_resolve_file_url() -> None:
    ...
    with patch("everos_cloud.lib._files.httpx.AsyncClient", return_value=mock_client):
